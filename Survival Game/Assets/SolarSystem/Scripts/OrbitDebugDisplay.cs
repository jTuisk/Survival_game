    ...
    public class OrbitDebugDisplay : MonoBehaviour
    {
        ...
        void DrawOrbits()
        {
            ...
            for (int step = 0; step < numSteps; step++)
            {
                ...
                for (int i = 0; i < virtualBodies.Length; i++)
                {
                    virtualBodies[i].velocity += CalculateAcceleration(i, virtualBodies) * timeStep * virtualBodies[i].velocityMultiplier;
                }
                // Update positions
                for (int i = 0; i < virtualBodies.Length; i++)
                {
        ...
        Vector3 CalculateAcceleration(int i, VirtualBody[] virtualBodies)
        {
            ...
            for (int j = 0; j < virtualBodies.Length; j++)
            {
                ...
                float m1 = virtualBodies[i].mass;
                float m2 = virtualBodies[j].mass;
                float sqrtR = (virtualBodies[j].position - virtualBodies[i].position).sqrMagnitude;
        ...
        class VirtualBody
        {
            ...
            public VirtualBody(CelestialBody body)
            {
                ...
                velocity = body.data.initialVelocity;
                velocityMultiplier = body.data.velocityMultiplier;
                mass = body.data.mass;
            }
        }
    }
