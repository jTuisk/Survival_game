...
public class ConditionalHidePropertyDrawer : PropertyDrawer
{
    ...
    bool CheckPropertyType(ConditionalHideAttribute condHAtt, SerializedProperty sourcePropertyValue)
    {
        ...
        switch (sourcePropertyValue.propertyType)
        {
            ...
            case SerializedPropertyType.Enum:
                return CheckIndexs(condHAtt, sourcePropertyValue);
    ...
    bool CheckIndexs(ConditionalHideAttribute condHAtt, SerializedProperty sourcePropertyValue)
    {
        bool display = false;

        if (condHAtt.enumIndexs != null)
        {
            for(int i = 0; i < condHAtt.enumIndexs.Length; i++)
            {
                if (sourcePropertyValue.enumValueIndex == condHAtt.enumIndexs[i])
                    display = true;
            }
        }
        else
        {
            display = sourcePropertyValue.enumValueIndex == condHAtt.enumIndex;
        }
